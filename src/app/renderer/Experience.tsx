...
import { Projects, type WorkExperience } from '../resume/Resume';
...
const ProjectEntry: React.FC<ProjectEntryProps> = ({ name, details, description, date, technology }) => {
  const title = name;
  return (
    <View style={styles.entryContainer}>
      <View style={styles.headerContainer}>
...
const Experience = ({ workExperience, projects }: { workExperience?: WorkExperience[]; projects?: Projects[] }) => {
  if (!workExperience) {
    return <p />;
  }
  return (
    <View style={styles.container}>
      ...
      {projects && projects.length > 0 && (
        <view>
          ...
          {projects.map(({ name, details, description, technologies, year }) => (
            <ProjectEntry name={name} details={details} technology={technologies} date={year} description={description} />
          ))}
        </view>
      )}
    </View>
